...
describe('ManifestJSONParser', () => {
  ...
  describe('manifestVersion', () => {
    ...
    it('should collect an error with unsupported new manifest_version', () => {
      const addonLinter = new Linter({ _: ['bar'] });
      const json = validManifestJSON({ manifest_version: 4 });
      const manifestJSONParser = new ManifestJSONParser(
        json,
        addonLinter.collector
      );
      expect(manifestJSONParser.isValid).toEqual(false);
      const { errors } = addonLinter.collector;
      expect(errors[0].code).toEqual(messages.MANIFEST_VERSION_INVALID.code);
      expect(errors[0].message).toContain('"manifest_version"');
    });
  });
